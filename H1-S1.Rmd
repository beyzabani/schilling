...
```{r echo=FALSE}
# Highest degree, Gender, and Nationality

df_degree_n <- df %>% 
   filter(Sample == 0 | Sample == 1) %>%
   drop_na(highest_degree, Nationality_co) %>% 
   mutate(Gender = factor(Gender, levels = c(0, 1), labels = c("Female", "Male")),
          highest_degree = factor(highest_degree, levels = c(1, 2, 3, 4), 
                                  labels = c("Bachelor's degree", "Master's degree/MBA", "PhD", "Other")),
          Nationality_co = factor(Nationality_co, levels = c(0, 1), labels = c("Schweiz", "Foreign")))
...
ggplot(df_summary, aes(x = Gender, y = count, fill = highest_degree)) +
  ...
  geom_text(aes(label = paste0(count, " (", round(percentage, 1), "%)")), 
            ...
            vjust = -0.5, 
            size = 3) +
  labs(title = "Bar Plot for Gender, Nationality, and Highest Degree", 
       x = "Gender", y = "Count") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap(~ Nationality_co)
...
# Plot with dodge position and both count and percentage labels
ggplot(df_summary, aes(x = Gender, y = count, fill = field)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = paste0(count, " (", round(percentage, 1), "%)")), 
            position = position_dodge(width = 0.9), 
            vjust = -0.5, 
            size = 3) +
  labs(title = "Bar Plot for Gender and Field of Study", x = "Gender", y = "Count") +
  theme_minimal() +
...
df_field_n <- df %>%
  filter(Sample == 0 | Sample == 1) %>%
  drop_na(fieldofstudy, Nationality_co) %>%
  mutate(field = case_when(
    fieldofstudy %in% c(0, 1, 2, 3, 4, 5) ~ "Economics and Management",
    fieldofstudy %in% c(6, 7, 8) ~ "STEM",
    fieldofstudy %in% c(9, 10, 11, 12) ~ "Other",
...
ggplot(df_summary, aes(x = Gender, y = count, fill = field)) +
  ...
  geom_text(aes(label = paste0(count, " (", round(percentage, 1), "%)")), 
            ...
            vjust = -0.5, 
            size = 3) +
  labs(title = "Bar Plot for Gender, Nationality, and Field of Study", 
       x = "Gender", y = "Count") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set3") +
  facet_wrap(~ Nationality_co)
