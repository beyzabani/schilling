...
df_age <- df %>% 
  ...
  mutate(prior_code_group = case_when(
    prior_code == 0 | prior_code == 1 | prior_code == 2 ~ "changed company",
    prior_code == 3 ~ "stayed in the same company",
    TRUE ~ NA_character_)) %>%  # Ensure all possible values are covered
  mutate(age_start = as.numeric(Stellenantritt) - as.numeric(Jahrgang)) %>% 
  mutate(Gender = factor(Gender, levels = c(0, 1), labels = c("Female", "Male"))) %>% 
...
# Plot age_start_group with prior_code_group and Gender
df_age_summary <- df_age %>%
  drop_na(prior_code_group, age_start_group) %>%  # Ensure no NA values disrupt the grouping
  group_by(Gender, prior_code_group, age_start_group) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  group_by(Gender, prior_code_group) %>%
  mutate(percentage = count / sum(count) * 100)
...
# Plot with dodge position and both count and percentage labels
ggplot(df_age_summary, aes(x = Gender, y = count, fill = age_start_group)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = paste0(count, " (", round(percentage, 1), "%)")), 
            position = position_dodge(width = 0.9), 
            vjust = -0.5, 
            size = 3) +
  labs(title = "Bar Plot for Gender, Prior Code Group, and Age Start Group", 
       x = "Gender", y = "Count") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap(~ prior_code_group)
...
# Plot age_exit_group with prior_code_group and Gender
df_age_summary <- df_age_exit %>%
  group_by(Gender, prior_code_group, age_exit_group) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  group_by(Gender, prior_code_group) %>%
  mutate(percentage = count / sum(count) * 100)
...
# Plot with dodge position and both count and percentage labels
ggplot(df_age_summary, aes(x = Gender, y = count, fill = age_exit_group)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = paste0(count, " (", round(percentage, 1), "%)")), 
            position = position_dodge(width = 0.9), 
            vjust = -0.5, 
            size = 3) +
  labs(title = "Bar Plot for Gender, Prior Code Group, and Age Exit Group", 
       x = "Gender", y = "Count") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap(~ prior_code_group)
