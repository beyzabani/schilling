...
#sum(is.na(df$tenure))
# Filter and process data for Sample 1
dfs1 <- df %>%
...
# Summarize statistics by gender and nationality
summary_stats1 <- dfs1 %>%
...
# Summarize statistics by gender and nationality, including count
summary_stats1 <- dfs1 %>%
  group_by(Gender, Nationality_co) %>%
  summarize(
...
summary_stats1 %>%
  kable(
    caption = "Sample 1: Average Tenure by Gender and Nationality",
