...
# Summary statistics by Gender and Employee Group
summary_stats_employee <- df_employee %>%
  group_by(Gender, employee_group) %>%
  summarize(
...
# Summary statistics table with count, average tenure, and SD tenure
summary_stats_employee %>%
  kable(
    caption = "Sample 2: Average Tenure by Gender and Employee Group",
    col.names = c("Gender", "Employee Group", "Count", "Average Tenure", "SD Tenure"),
    align = "c"
  ) %>%
  kable_styling(
